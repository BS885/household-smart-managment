...
namespace SmartManagement.Service.Services
{
    public class ExpenseService : IExpenseService
    {
        ...
        public ExpenseService(IExpenseRepository expenseRepository, ILogger<ExpenseService> logger, IMapper mapper, ICategoryRepository categoryRepository)
        {
        ...
        public async Task<int> AddExpenseAsync(DateTime date, string category, int userId, string description, TransactionType typeTransaction, decimal sum, int? fixedExpenseAndIncomeId)
        {
            try
            {
               ...
               var re= await _expenseRepository.AddExpenseAsynce(expense); // לוודא שזו פעולה אסינכרונית

                _logger.LogInformation("Expense added successfully");

                return expense.Id; // כעת ה-Id אמור להיות מעודכן מה-DB
            }
            catch (Exception ex)
            {
